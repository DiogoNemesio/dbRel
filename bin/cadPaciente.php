...
$grid->adicionaIcone('icon-camera','Adicionar a Foto');
$grid->adicionaIcone('icon-file-alt','Questionário Auxiliar');
...
for ($i = 0; $i < sizeof($gridData); $i++) {
	...
	$grid->setValorColuna($i,6,BIN_URL.'/editPaciente.php?id='.$id);
	$grid->setValorColuna($i,7,BIN_URL.'/excPaciente.php?id='.$id);
	$grid->setValorColuna($i,8,BIN_URL.'/pacienteFoto.php?id='.$id);
	$grid->setValorColuna($i,9,"javascript:mostraQuestionario(\"".$gridData[$i]->CODIGO."\",\"\",\"A\",\"\");");
}
...
$template->assign('NOME'				,'Pacientes');
