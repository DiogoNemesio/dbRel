...
#################################################################################
//$gridData		= usuario::lista(null,'V');
$gridData		= usuario::listaVoluntarios();
...
for ($i = 0; $i < sizeof($gridData); $i++) {
	...
	$grid->setValorColuna($i,5,BIN_URL.'/editVoluntario.php?id='.$id);
	$grid->setValorColuna($i,6,BIN_URL.'/excVoluntario.php?id='.$id);
...
#################################################################################
$urlAdd			= BIN_URL.'editVoluntario.php?id='.DHCUtil::encodeUrl('_codMenu_='.$_codMenu_.'&codUsuario='); 
...
$template->assign('NOME'				,'Voluntários');
